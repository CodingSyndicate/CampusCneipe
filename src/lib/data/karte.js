export let karte = [
	{	// Bier & Wein
		name: 'Bier & Wein',
		image: 'https://images.unsplash.com/photo-1518791841217-8f162f1e1131?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60',
		type: 'drinks',
		color: '#152355',
		content: [
			{
				name: 'Helles',
				subtitle: 'vom Fass',
				description: 'auch Alkoholfrei',
				amount: '0,5l / 1,0l',
				price: '3,30 / 6,10'
			},
			{
				name: 'Weißbier',
				subtitle: 'vom Fass',
				description: 'auch Alkoholfrei',
				amount: '0,5l',
				price: '3,30'
			},
			{
				name: 'Vitus',
				subtitle: 'vom Fass',
				description: null,
				amount: '0,5l',
				price: '3,60'
			},
			{
				name: 'Helles',
				subtitle: null,
				description: null,
				amount: '0,5l',
				price: '3,30'
			},
			{
				name: 'Dunkles Weißbier',
				subtitle: null,
				description: null,
				amount: '0,5l',
				price: '3,30'
			},
			{
				name: 'Pils',
				subtitle: null,
				description: null,
				amount: '0,33l',
				price: '2,90'
			},
			{
				name: 'Radler',
				subtitle: null,
				description: null,
				amount: '0,5l / 1,0l',
				price: '3,30 / 6,10'
			},
			{
				name: 'Russn / Colaweizen',
				subtitle: null,
				description: null,
				amount: '0,5l',
				price: '3,30'
			},
			{
				name: 'Isarwasser',
				subtitle: null,
				description: null,
				amount: '0,5l',
				price: '4,20'
			},
			{
				name: 'Vitusse',
				subtitle: null,
				description: null,
				amount: '0,5l',
				price: '3,50'
			},
			{
				name: 'Goaßmaß',
				subtitle: null,
				description: null,
				amount: '1,0l',
				price: '6,60'
			}
		]
	},
	{	// Classics
		name: 'Classics',
		image: 'https://images.unsplash.com/photo-1518791841217-8f162f1e1131?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60',
		type: 'drinks',
		color: '#0A537C',
		content: [
			{
				name: 'Fresh Hermann',
				subtitle: null,
				description: 'Der TUM Klassiker',
				amount: '0,5l',
				price: '6,30'
			},
			{
				name: 'Tequila Sunrise',
				subtitle: null,
				description: 'Tequila, Zitronensaft, Orangensaft, Grenadine',
				amount: '-',
				price: '6,30'
			},
			{
				name: 'Caipirinha',
				subtitle: null,
				description: 'Limette, Rohrzucker, Cachaca, Soda',
				amount: '-',
				price: '6,30'
			},
			{
				name: 'Touch Down',
				subtitle: null,
				description: 'Vodka, Apricot, Brandy, Zitronensaft, Grenadine, Maracujanektar',
				amount: '-',
				price: '6,30'
			},
			{
				name: 'Sex on the Beach',
				subtitle: null,
				description: 'Vodka, Pfirsichlikör, Orangensaft, Cranberrynektar',
				amount: '-',
				price: '6,30'
			},
			{
				name: 'Mojito',
				subtitle: null,
				description: 'Limette, Rohrzucker, Havana Club Rum, Minze, Soda',
				amount: '-',
				price: '6,30'
			},
			{
				name: 'Negroni',
				subtitle: null,
				description: 'Gin, Campari, Vermouth',
				amount: '-',
				price: '6,30'
			},
			{
				name: 'Pina Colada',
				subtitle: null,
				description: 'Cream of Coconut, Ananassaft, Maracujanektar, dunkler Rum',
				amount: '-',
				price: '6,30'
			}
		]
	},
	{	// Longdrinks Light
		name: 'Longdrinks Light',
		image: 'https://images.unsplash.com/photo-1518791841217-8f162f1e1131?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60',
		type: 'drinks',
		color: '#3B2872',
		content: [
			{
				name: 'Vodka Bull',
				subtitle: null,
				description: 'Absolut Vodka, Red Bull',
				amount: '-',
				price: '7,60'
			},
			{
				name: 'Cuba Libre',
				subtitle: null,
				description: 'Havanna Club Rum, Limette, Coca Cola',
				amount: '-',
				price: '5,90'
			},
			{
				name: 'Gin Tonic',
				subtitle: null,
				description: 'Bombay Saphire Gin, Tonic Water',
				amount: '-',
				price: '6,30'
			},
			{
				name: 'Moscow Mule / Munich Mule',
				subtitle: null,
				description: 'Vodka oder Gin, Limettensaft, Gingerbeer',
				amount: '-',
				price: '6,30'
			},
			{
				name: 'Perfect Storm',
				subtitle: null,
				description: 'Kraken, Limettensaft, Gingerbeer',
				amount: '-',
				price: '6,30'
			}
		]
	},
	{	// Longdrinks Strong
		name: 'Longdrinks Strong',
		image: 'https://images.unsplash.com/photo-1518791841217-8f162f1e1131?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60',
		type: 'drinks',
		color: '#420D4A',
		content: [
			{
				name: 'Long Island Iced Tea',
				subtitle: null,
				description: 'heller Rum, Vodka, Bombay Gin, Cointreau, Zitronensaft, Rohrzuckersirup, Coca Cola',
				amount: '-',
				price: '7,60'
			},
			{
				name: 'Zombie',
				subtitle: null,
				description: 'dunkler Rum, heller Rum, Limettensaft, Apricot Brandy, Grenadine, Orangensaft, Ananassaft',
				amount: '-',
				price: '7,60'
			},
			{
				name: 'Mai Tai',
				subtitle: null,
				description: 'dunkler Rum, heller Rum, Limettensaft, Cointreau, Mandelsirup, Rohrzuckersirup',
				amount: '-',
				price: '7,60'
			}
		]
	},
	{	// Sprizz
		name: 'Sprizz',
		image: 'https://images.unsplash.com/photo-1518791841217-8f162f1e1131?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60',
		type: 'drinks',
		color: '#650F29',
		content: [
			{
				name: 'Aperol Sprizz',
				subtitle: null,
				description: 'Aperol, Prosecco, Soda',
				amount: '-',
				price: '4,60'
			},
			{
				name: 'Limoncello Sprizz',
				subtitle: null,
				description: 'Limoncello, Prosecco, Soda',
				amount: '-',
				price: '4,60'
			},
			{
				name: "Pimm's Cup",
				subtitle: null,
				description: "Pimm's No 1, Ginger Ale",
				amount: '-',
				price: '6,30'
			}
		]
	},
	{	// Fizzes & Sours
		name: 'Fizzes & Sours',
		image: 'https://images.unsplash.com/photo-1518791841217-8f162f1e1131?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60',
		type: 'drinks',
		color: '#3B2872',
		content: [
			{
				name: 'Gin Basil Smash',
				subtitle: null,
				description: 'Basilikum, Gin, Zitronensaft',
				amount: '-',
				price: '6,30'
			},
			{
				name: 'Fizz',
				subtitle: null,
				description: 'Spirit, Zitronensaft, Rohrzuckersirup, Soda',
				amount: '-',
				price: '6,30'
			},
			{
				name: 'Sour',
				subtitle: null,
				description: 'Spirit, Zitronensaft, Rohrzuckersirup',
				amount: '-',
				price: '6,30'
			}
		]
	},
	{	// Mocktails
		name: 'Mocktails',
		image: 'https://images.unsplash.com/photo-1518791841217-8f162f1e1131?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60',
		type: 'drinks',
		color: '#40099A',
		content: [
			{
				name: 'Ginger Basil Smash',
				subtitle: null,
				description: 'Basilikum, Grapefruitsaft, Gingerbeer',
				amount: '-',
				price: '5,30'
			},
			{
				name: 'Mint Tonic',
				subtitle: null,
				description: 'Rohrzuckersirup, Zitrone, Minzzweig, Tonic Water',
				amount: '-',
				price: '5,30'
			},
			{
				name: 'Crodino Ale / Ginger Fizz',
				subtitle: null,
				description: 'Crodino, Ginger Ale | Ginger Beer, Soda',
				amount: '-',
				price: '6,30'
			},
			{
				name: 'Caipinino',
				subtitle: null,
				description: 'Crodino, Ginger Ale | Ginger Beer, Soda',
				amount: '-',
				price: '5,30'
			},
			{
				name: 'PiNADA Colada',
				subtitle: null,
				description: 'Cream of Coconut, Ananassaft, Maracujanektar, Limettensaft',
				amount: '-',
				price: '5,30'
			}
		]
	},
	{	// Shots
		name: 'Shots',
		image: 'https://images.unsplash.com/photo-1518791841217-8f162f1e1131?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60',
		type: 'drinks',
		color: '#420D4A',
		content: [
			{
				name: 'TUM Kamikaze',
				subtitle: null,
				description: 'Vodka, Limettensaft, Blue Curacao',
				amount: '-',
				price: '3,60'
			},
			{
				name: 'Wochenschnaps',
				subtitle: null,
				description: 'wöchentlich ausgewählter Schnaps',
				amount: '-',
				price: '1,00'
			},
			{
				name: 'B52',
				subtitle: null,
				description: 'Khalua, Balleys, Overproof Rum',
				amount: '-',
				price: '3,90'
			},
			{
				name: 'Liquid Cocaine',
				subtitle: null,
				description: 'Vodka, Khalua, Puderzucker',
				amount: '-',
				price: '3,90'
			},
			{
				name: 'Mexicana',
				subtitle: null,
				description: 'hausgemachter Mexicanamix, Tabasco, Pfeffer',
				amount: '-',
				price: '3,60'
			},
			{
				name: 'Apfelstrudel',
				subtitle: null,
				description: 'Apfelsaft, Malibu, Zimt',
				amount: '-',
				price: '3,10'
			},
			{
				name: 'Classics',
				subtitle: null,
				description: 'Vodka, Tequila (dunkel/hell), Ouzo, Jägermeister, Limoncello, Berliner Luft, Frangelico, Willi, Sambuca, Blutwurz, Ramazotti',
				amount: '-',
				price: '2,00'
			},
			{
				name: 'Berliner Luft (Flasche)',
				subtitle: null,
				description: '',
				amount: '-',
				price: '22,00'
			}
		]
	},
	{	// Alkoholfreie Getränke
		name: 'Alkoholfreie Getränke',
		image: 'https://images.unsplash.com/photo-1518791841217-8f162f1e1131?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60',
		type: 'drinks',
		color: '#0275A7',
		content: [
			{
				name: 'Säfte / Nektar',
				subtitle: null,
				description: 'Apfel, Ananas, Orange, Johannesbeere, Maracuja',
				amount: '0,4l',
				price: '1,50'
			},
			{
				name: 'Saftschorlen',
				subtitle: null,
				description: 'Apfel, Ananas, Orange, Johannisbeer, Maracuja',
				amount: '0,5l',
				price: '1,50'
			},
			{
				name: 'Zitronenlimonade',
				subtitle: null,
				description: null,
				amount: '0,5l',
				price: '1,50'
			},
			{
				name: 'Fanta',
				subtitle: null,
				description: null,
				amount: '0,5l',
				price: '1,50'
			},
			{
				name: 'Spezi',
				subtitle: null,
				description: null,
				amount: '0,5l',
				price: '1,50'
			},
			{
				name: 'Cola',
				subtitle: null,
				description: null,
				amount: '0,4l',
				price: '1,50'
			},
			{
				name: 'Clubmate',
				subtitle: null,
				description: null,
				amount: '0,5l',
				price: '1,50'
			},
			{
				name: 'Tafelwasser',
				subtitle: null,
				description: null,
				amount: '0,5l',
				price: '1,20'
			},
			{
				name: 'Red Bull',
				subtitle: null,
				description: null,
				amount: '0,33l',
				price: '3,40'
			},
			{
				name: 'Schweppes',
				subtitle: null,
				description: 'Ginger Ale, Ginger Beer, Tonic Water',
				amount: '0,2l',
				price: '1,00'
			},
			{
				name: 'Heiße Getränke',
				subtitle: null,
				description: 'Kaffee, Heiße Zitrone, Tee',
				amount: '-',
				price: '2,00'
			},
			{
				name: 'Espresso',
				subtitle: null,
				description: null,
				amount: '-',
				price: '1,80'
			}
		]
	},
];
